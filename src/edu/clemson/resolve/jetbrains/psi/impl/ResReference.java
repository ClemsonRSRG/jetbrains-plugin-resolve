...
public class ResReference
    ...
    @Nullable
    public static ResTypeReferenceExp getTypeReference(
            @Nullable ResType o) {
    ...
    protected static boolean processModuleLevelEntities(@NotNull ResFile file,
        ...
        if (!processNamedElements(processor, state, file.getOperationLikeThings(), localProcessing))
            return false;
        if (!processNamedElements(processor, state, file.getFacilities(), localProcessing))
            return false;
        if (!processNamedElements(processor, state, file.getTypes(), localProcessing))
            return false;
        if (!processNamedElements(processor, state, file.getGenericTypeParams(), localProcessing))
            return false;
        if (!processNamedElements(processor, state, file.getMathDefinitionSignatures(), localProcessing))
            return false;
        return true;
    }
    ...
    private static boolean processDefinitionParams(
        ...
        for (ResMathVarDeclGroup declaration : parameters) {
            if (!processNamedElements(processor, ResolveState.initial(), declaration.getMathVarDefList(), true))
                return false;
    ...
    @NotNull
    private ResVarReference.ResVarProcessor createDelegate(
            @NotNull ResScopeProcessor processor) {
    ...
    @Nullable
    protected static PsiFile getContextFile(
            @NotNull ResolveState state) {
